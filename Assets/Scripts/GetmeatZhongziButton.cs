...
public class GetmeatZhongziButton : MonoBehaviour
{
    // 按钮组件（需在Inspector关联）
    ...
    void OnButtonClicked()
    {
        ...
        {
            case "good":
                itemManager.AddPromeatZhongzi(); // 替换为肉粽子的高级添加方法
                Debug.Log("按钮触发：当前状态为good，已添加pro肉粽子");
                break;
            case "ordinary":
                itemManager.AddmeatZhongzi(); // 替换为肉粽子的普通添加方法
                Debug.Log("按钮触发：当前状态为ordinary，已添加肉粽子");
                break;
            case "bad":
