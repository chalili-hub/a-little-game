...
public class GetRedBeanZhongziButton : MonoBehaviour
{
    // 按钮组件（需在Inspector关联）
    ...
    void OnButtonClicked()
    {
        ...
        {
            case "good":
                itemManager.AddProRedBeanZhongzi(); // 替换为红豆粽子的高级添加方法
                Debug.Log("按钮触发：当前状态为good，已添加pro红豆粽子");
                break;
            case "ordinary":
                itemManager.AddRedBeanZhongzi(); // 替换为红豆粽子的普通添加方法
                Debug.Log("按钮触发：当前状态为ordinary，已添加红豆粽子");
                break;
            case "bad":
